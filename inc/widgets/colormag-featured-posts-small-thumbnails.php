...
class colormag_featured_posts_small_thumbnails extends WP_Widget {
	...
	function __construct() {
		...
		parent::__construct( false, $name = __( 'TG: Featured Posts (Style 3)', 'colormag-pro' ), $widget_ops );
	}
	...
	function form( $instance ) {
		...
		<p>
			...
			<label for="<?php echo $this->get_field_id( 'ajax_load_more' ); ?>"><?php _e( 'Check to display the ajax load more button to load further posts from chosen category or from latest post.', 'colormag-pro' ); ?></label>
		</p>
    ...
    <use href="/wp-content/themes/colormag-pro/fonts/fontawesome.svg#comment-solid"></use>
		...
		if ( $ajax_load_more == 'true' ) {
			...
			<div class="tg-ajax-btn-wrapper">
				...
				<div class="tg-front-post-load-more btn-wrapper" id="tg-ajax-btn-<?php echo esc_attr( $this->id ); ?>" data-category='<?php echo esc_attr( $category ); ?>' data-child_category='<?php echo absint( $child_category_display ); ?>' data-random="<?php echo $random_posts; ?>" data-number="<?php echo esc_attr( $number ); ?>" data-tag="<?php echo esc_attr( $tag ); ?>" data-type="<?php echo esc_attr( $type ); ?>">
					<span class="transition5 btn btn-view-all"><?php esc_html_e( 'Load More', 'colormag-pro' ); ?></span>
