...
add_action('wp_ajax_nopriv_push_to_sendy_ajax', 'push_to_sendy_ajax');
...
function send_campaign_to_sendy($post_id, $email_data) {
	...
	// Check if campaign exists using WordPress HTTP API
	...
	if (is_wp_error($response)) {
		error_log('Sendy campaign check failed: ' . $response->get_error_message());
		return false;
	}
	
	$exists = wp_remote_retrieve_body($response);
	...
	// Send or update the campaign using WordPress HTTP API
	$response = wp_remote_post($sendy_url, array(
		'headers' => array(
			'Content-Type' => 'application/x-www-form-urlencoded'
		),
	...
	if (is_wp_error($response)) {
		error_log('Sendy campaign send/update failed: ' . $response->get_error_message());
		return false;
	}
...
function extrachill_submit_newsletter_form() {
	...
	if ($response_body === '1' || strpos($response_body, 'Success') !== false) {
		...
		wp_send_json_error('Failed to subscribe, Sendy response: ' . $response_body);
	}
...
function enqueue_newsletter_popup_scripts() {
	...
	// Exclude: Contact page (assuming slug 'contact-us', adjust if needed)
	...
	if ( $load_script ) {
		...
		if ( file_exists( $script_path ) ) {
			wp_enqueue_script('custom-popup-subscribe', $script_uri, array('jquery'), filemtime( $script_path ), true);
			wp_localize_script('custom-popup-subscribe', 'newsletter_vars', array(
...
function extrachill_subscribe_to_sendy_home() {
	check_ajax_referer('subscribe_to_sendy_home_nonce', 'nonce');
	$email = sanitize_email($_POST['email']);
	$sendy_url = 'https://mail.extrachill.com/sendy';
	$list_id = 'yYBiTvmXZH892P763BS9MOHalQ'; // TODO: Replace with actual Sendy list ID for homepage
	$api_key = 'z7RZLH84oEKNzMvFZhdt';
	$postData = array(
		'email' => $email,
		'list' => $list_id,
...
add_action('wp_ajax_subscribe_to_sendy_home', 'extrachill_subscribe_to_sendy_home');
add_action('wp_ajax_nopriv_subscribe_to_sendy_home', 'extrachill_subscribe_to_sendy_home');
